...
between the hemoglobin A1c (HbA1c, an important biomarker for type 2 diabetes)
levels and the beta cell functions (Kanat et al., 2011, Hou et al., 2016). 
...
Empirically, a good threshold value would cut the number of genes to 1000, which we believe is an adequate number to detect the inter-cell-type variations.



## Mouse Mammary Gland Data Analysis
Using the single cell clustering package [Seurat](https://satijalab.org/seurat/), we classified the mouse mammary gland single cells into five cell types of interest: endothelial, fibroblast, luminal, basal and immune cells.
Then we perform deconvolution for mouse mammary gland tissue bulk RNA-seq data of two forms: fresh-frozen and 10X. The detailed data process structure is shown in the figure 5 of our paper.
