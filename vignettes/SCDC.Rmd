...
```
The choice of limit for LogFoldChange `LFC.lim=5` could be changed by users. A more robust way is to select a reasonable range of threshold, for example, in this case, we choose from 3 to 6, and then estimate the proportions using different threshold. Say we calculated proportions from 3 to 6 by 0.1, we will derive 30 different estimated proportions results. We can calculate the mean from these results, or we can take the median of the value of each estimated cell type, and normalize the sample-wise proportions to 1. Empirically, a good threshold value would cut the number of genes to 1000, which we believe is an adequate number to detect the inter-cell-type variations.
